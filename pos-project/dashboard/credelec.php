...
$stmt = $db->prepare(
    "SELECT * FROM TblElectricity WHERE uid = ? AND DATE(createdAt) >= ? AND DATE(createdAt) <= ? ORDER BY createdAt DESC"
);
    ...
    <div class="col-12">
      <div class="card mb-4">
        <div class="card-header pb-0">
          <h6>Transacções Credelec — <?= date('d/m/Y', strtotime($dateFrom)) ?> a <?= date('d/m/Y', strtotime($dateTo)) ?></h6>
        </div>
          ...
          <div class="table-responsive p-0">
            <table class="table align-items-center mb-0">
              ...
              <tbody>
                ...
                <tr>
                  <td class="ps-4">
                    <p class="text-xs font-weight-bold mb-0"><?= htmlspecialchars($t['meterNumber']) ?></p>
                  ...
                  <td class="align-middle text-center">
                    <span class="text-xs font-weight-bold text-success"><?= htmlspecialchars($t['token'] ?? '—') ?></span>
                  </td>
